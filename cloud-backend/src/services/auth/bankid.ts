...
class BankIDAuthService {
  ...
  async initiateAuth(personalNumber?: string): Promise<SSOSession> {
    try {
      ...
      const response = await apiClient.post<BankIDInitResponse>('/auth/bankid/initiate', request);
  ...
  async pollStatus(sessionId: string, onStatusChange?: (status: SSOSession) => void): Promise<SSOSession> {
    ...
    return new Promise((resolve) => {
      this.pollingTimer = setInterval(async () => {
        attempts++;
        try {
          const response = await apiClient.get<BankIDStatusResponse>(`/auth/bankid/status/${sessionId}`);
  ...
  async cancelAuth(sessionId: string): Promise<void> {
    try { this.stopPolling(); await apiClient.post(`/auth/bankid/cancel/${sessionId}`); }
  ...
  async completeAuth(sessionId: string): Promise<{ success: boolean; token?: string; user?: any; error?: string }> {
    try {
      const response = await apiClient.post('/auth/bankid/complete', { sessionId });
